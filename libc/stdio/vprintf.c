...
inline static bool _print_fspec_chr(_fspec *fspec) {
    const char ch = fspec->tspec.chr_spec.ch;
    return print(&ch, sizeof(ch));
}

inline static bool _print_fspec_str(_fspec *fspec) {
    const char* str = fspec->tspec.str_spec.str;
    return print(str, fspec->len);
}

inline static bool _print_fspec_int(_fspec *fspec) {
...
inline static bool _print_fspec_ptr(_fspec *fspec) {
...
static bool _print_fspec_type(_fspec* fspec) {
    if (fspec->type == FT_CHR)
        return _print_fspec_chr(fspec);
    if (fspec->type == FT_STR)
        return _print_fspec_str(fspec);
    if (fspec->type == FT_INT)
        return _print_fspec_int(fspec);
    if (fspec->type == FT_PTR)
        return _print_fspec_ptr(fspec);

    // Should never be reached
    return false;
}

static bool _print_fspec(_fspec* fspec) {
    size_t pad = compute_pad(fspec);

    bool is_left_pad = !_is_left_flag(fspec) && printpad(pad);
    bool is_print = _print_fspec_type(fspec);
    bool is_right_pad = _is_left_flag(fspec) && printpad(pad);
